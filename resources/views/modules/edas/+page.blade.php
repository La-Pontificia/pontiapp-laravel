...
@section('layout.edas')
    <div class="w-full max-w-7xl mx-auto">
        <h2 class="py-5">Colaboradores bajo tu supervisión</h2>
        <div
            class="flex flex-col w-full overflow-auto bg-white border-neutral-300 shadow-[0_0_10px_rgba(0,0,0,.2)] border rounded-xl">
                ...
                @if ($cuser->has('users:edas:show') || $cuser->isDev())
                    @forelse ($users as $user)
                        <div class="flex relative items-center p-3 gap-2">
                            ...
                            <div class="flex-grow">
                                <p>
                                    <a href="/users/{{ $user->id }}"
                                        title="Ver perfil de {{ $user->first_name . ' ' . $user->last_name }}"
                                        class="hover:underline text-nowrap hover:text-blue-700">
                            ...
                            @if ($user->supervisor)
                                <p class="text-nowrap">
                                    Bajo supervisión de <a href="/users/{{ $user->supervisor->id }}"
                                        class="hover:underline text-nowrap hover:text-blue-700">
