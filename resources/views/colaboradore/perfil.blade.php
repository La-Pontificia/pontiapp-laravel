<div id="perfil-modal-{{ $colaborador->id }}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-2xl shadow :bg-gray-700">
            <button type="button"
                ...
                data-modal-hide="perfil-modal-{{ $colaborador->id }}">
            ...
            <div class="px-6 py-6 flex flex-col items-center lg:px-8">
                ...
                <div class="flex relative group min-w-[20rem] h-[20rem] w-[20rem] border overflow-hidden rounded-full"">
                    <img alt="{{ $colaborador->nombres }}" src="{{ $colaborador->perfil }}"
                        class="w-full h-full object-cover imagePreview" />
                    <span
                        class="absolute btnchangeimage group-hover:opacity-100 opacity-0 cursor-pointer inset-0 bg-white/80 grid place-content-center">
                    ...
                    <input type="file" hidden class="inputchangeimage" accept="image/*" />
                </div>
                <button data-id={{ $colaborador->id }} data-dni="{{ $colaborador->dni }}"
                    class="bg-slate-800 upload-btn mt-3 hidden disabled:opacity-50 p-1 rounded-full text-white font-semibold px-3">Actualizar
                    perfil</button>
            </div>
        </div>
    </div>
