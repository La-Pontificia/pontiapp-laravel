...
@section('content-meta')
    @php
        ...
        $habilitareva1 = !$edaSeleccionado->evaluacion->cerrado;
        $habilitareva2 = !$edaSeleccionado->evaluacion2->cerrado;

        $eva_1 = $edaSeleccionado->evaluacion;
        $eva_2 = $edaSeleccionado->evaluacion2;

        $cerrareda = $edaSeleccionado->evaluacion->cerrado && $edaSeleccionado->evaluacion2->cerrado && $aprobado && $suSupervisor;
        ...
        $habilitar_eva_1 = $aprobado;
        $habilitar_eva_2 = $eva_1->cerrado;
        $eliminarEda = $colaborador_actual->rol == 2;
    @endphp
            ...
            <div class="flex w-[550px] max-md:w-full flex-col gap-2">
                <a class="{{ $habilitarobjetivos ? '' : 'opacity-50 select-none pointer-events-none' }} group"
                    ...
                    <div class="p-3 border items-center rounded-2xl flex gap-2 group-hover:bg-neutral-100">
                        ...
                        @if ($aprobado)
                            <h1
                                class="p-1 min-w-max flex items-center text-sm rounded-2xl text-white bg-green-400 font-medium">
                ...
                <a href="/meta/{{ $id_colab }}/eda/{{ $id_eda }}/1"
                    class="group {{ !$habilitar_eva_1 ? 'opacity-40 grayscale select-none pointer-events-none' : '' }}">
                ...
                <a href="/meta/{{ $id_colab }}/eda/{{ $id_eda }}/2"
                    class="group {{ !$habilitar_eva_2 ? 'opacity-40 grayscale select-none pointer-events-none' : '' }}">
                    <div class="p-3 border rounded-2xl flex gap-2 group-hover:bg-neutral-100">
                        <div class="flex gap-3 items-center w-full">
                            <div class="bg-blue-500 p-4 text-white rounded-xl">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" version="1.1"
