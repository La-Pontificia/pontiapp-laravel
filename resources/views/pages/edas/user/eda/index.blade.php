        ...
        @if ($eda)
            <div>
                <div class="px-2">
                    ...
                    <p class="opacity-60 font-normal text-sm">Creado el
                        {{ \Carbon\Carbon::parse($eda->created_at)->isoFormat('LL') }}</p>
                </div>
                <div class="p-2 flex relative">
                    ...
                    <div class="py-5 w-[500px]  space-y-5">
                        <a href="{{ route('edas.user.eda.goals', ['year' => $year->id, 'id_user' => $user->id]) }}"
                            class="border p-2 gap-2 items-center rounded-lg bg-white hover:border-rose-500 w-full flex">
                            ...
                            <span
                                class="absolute w-5 rounded-full border-4 block aspect-square left-3.5 border-indigo-500 bg-white"></span>
                        </a>
                        ...
                        @foreach ($evaluations as $index => $evaluation)
                            ...
                            <a href="{{ route('edas.user.eda.evaluation', ['year' => $year->id, 'id_user' => $user->id, 'id_evaluation' => $evaluation->id]) }}"
                                ...
                                class="border data-[hidden]:opacity-30 data-[hidden]:pointer-events-none p-2 gap-2 items-center rounded-lg bg-white hover:border-blue-500 w-full flex">
                                ...
                                <span {{ !$eda->approved || !$prevEvaluation->closed ? 'data-hidden' : '' }}
                                    class="absolute w-5 rounded-full data-[hidden]:grayscale border-4 block aspect-square left-3.5 border-indigo-500 bg-white"></span>
                            </a>
                        @endforeach
                    </div>
                </div>
